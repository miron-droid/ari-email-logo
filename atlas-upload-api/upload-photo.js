...
uploadFile.addEventListener('change',async()=>{
 ...
 const accessCode=document.getElementById('upload-code').value.trim();if(!accessCode){uploadStatus.textContent='Enter your team upload code first.';uploadFile.value='';return;}
 ...
 try{
  ...
  uploadStatus.textContent='Saving photo to GitHub…';const response=await fetch(UPLOAD_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+accessCode},body:JSON.stringify({image:canvas.toDataURL('image/jpeg',0.88)}),signal:AbortSignal.timeout(45000)});const result=await response.json();if(!response.ok)throw Error(result.error||'Upload failed.');
